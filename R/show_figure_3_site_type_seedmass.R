...
# Show figure of seed mass ~ EUNIS x site.type
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Markus Bauer
# 2024-05-09
...
medians <- sites %>%
  ...
  summarise(median = median(cwm.abu.seedmass)) %>%
...
p1 <- ggplot(
  sites, aes(y = cwm.abu.seedmass, x = reference, fill = reference)
  ) +
  ...
  labs(y = "CWM Seed mass (abu) [g]", tag = "C") +
  theme_mb() +
...
ggsave(
  here(
    "outputs", "figures",
    "figure_cwm_eunis_2_site.type_seedmass_300dpi_17x10cm.tiff"
  ),
