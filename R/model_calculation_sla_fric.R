...
ggplot(sites, aes(y = y, x = esy4)) +
  ...
  labs(y = "FRic SLA (abu) [-]", x = "Habitat type")

ggplot(sites, aes(y = y, x = eco.id)) +
  geom_quasirandom(color = "grey") +
  geom_boxplot(fill = "transparent") +
  facet_grid(~ esy4) +
  labs(y = "FRic SLA (abu) [-]", x = "Ecoregion")

ggplot(sites, aes(y = y, x = site.type)) +
  geom_quasirandom(color = "grey") +
  geom_boxplot(fill = "transparent") +
  facet_grid(~ esy4) +
  labs(y = "FRic SLA (abu) [-]", x = "Site type")

ggplot(sites, aes(y = y, x = obs.year)) +
  geom_quasirandom(color = "grey") +
  geom_boxplot(fill = "transparent") +
  facet_grid(~ esy4) +
  labs(y = "FRic SLA (abu) [-]", x = "Survey year")
...
save(m1, file = here("outputs", "models", "model_sla_esy4_fric_1.Rdata"))
save(m2, file = here("outputs", "models", "model_sla_esy4_fric_2.Rdata"))
